...
public class AtmSyncLogBean implements Serializable {
    ...
    /**
     * <pre>
     * DB remark: ATM终端，上次同步成功的记录时间，【此值为日期时间格式，考虑各ATM客户系统的日期格式差异，目前comodin系统，进行统一处理：yyyy-MM-dd HH:mm:ss.SSS，日期时区服务器不做任务处理，以ATM客户为主】
     * DB column: sync_last_time	VARCHAR(23)	<--->	lastTime	java.lang.String
     * DB is  Nullable: false
     * DB defaultValue: null
     * </pre>
     */
    ...
    @Length(max = 23, message = "{" + AtmSyncLogBeanI18nConstant.ATM_SYNC_LOG_BEAN_LAST_TIME_LENGTH + "}", groups = {IBaseValidGroup.Add.class, IBaseValidGroup.Update.class})
    @Column(name = "sync_last_time", length = 23, nullable = false)
    private String lastTime;

    /**
     * <pre>
     * DB remark: ATM终端，上次同步成功的主键值，此字段为了解决，ATM终端系统，对应的同步表中，生成的记录的创建时间相同，主键ID不同问题
     * DB column: sync_last_primary_key	VARCHAR(20)	<--->	lastPrimaryKey	java.lang.String
     * DB is  Nullable: true
     * DB defaultValue: null
     * </pre>
     */
    @Length(max = 20, message = "{" + AtmSyncLogBeanI18nConstant.ATM_SYNC_LOG_BEAN_LAST_PRIMARY_KEY_LENGTH + "}", groups = {IBaseValidGroup.Add.class, IBaseValidGroup.Update.class})
    @Column(name = "sync_last_primary_key", length = 20, nullable = true)
    ...
    /**
     * 获取 ATM终端，上次同步成功的记录时间，【此值为日期时间格式，考虑各ATM客户系统的日期格式差异，目前comodin系统，进行统一处理：yyyy-MM-dd HH:mm:ss.SSS，日期时区服务器不做任务处理，以ATM客户为主】
     *
     * @return sync_last_time - ATM终端，上次同步成功的记录时间，【此值为日期时间格式，考虑各ATM客户系统的日期格式差异，目前comodin系统，进行统一处理：yyyy-MM-dd HH:mm:ss.SSS，日期时区服务器不做任务处理，以ATM客户为主】
     */
    ...
    /**
     * 设置 ATM终端，上次同步成功的记录时间，【此值为日期时间格式，考虑各ATM客户系统的日期格式差异，目前comodin系统，进行统一处理：yyyy-MM-dd HH:mm:ss.SSS，日期时区服务器不做任务处理，以ATM客户为主】
     *
     * @param lastTime - ATM终端，上次同步成功的记录时间，【此值为日期时间格式，考虑各ATM客户系统的日期格式差异，目前comodin系统，进行统一处理：yyyy-MM-dd HH:mm:ss.SSS，日期时区服务器不做任务处理，以ATM客户为主】
     */
    ...
    /**
     * 获取 ATM终端，上次同步成功的主键值，此字段为了解决，ATM终端系统，对应的同步表中，生成的记录的创建时间相同，主键ID不同问题
     *
     * @return sync_last_primary_key - ATM终端，上次同步成功的主键值，此字段为了解决，ATM终端系统，对应的同步表中，生成的记录的创建时间相同，主键ID不同问题
     */
    ...
    /**
     * 设置 ATM终端，上次同步成功的主键值，此字段为了解决，ATM终端系统，对应的同步表中，生成的记录的创建时间相同，主键ID不同问题
     *
     * @param lastPrimaryKey - ATM终端，上次同步成功的主键值，此字段为了解决，ATM终端系统，对应的同步表中，生成的记录的创建时间相同，主键ID不同问题
     */
    public AtmSyncLogBean setLastPrimaryKey(String lastPrimaryKey) {
        this.lastPrimaryKey = lastPrimaryKey == null ? null : lastPrimaryKey.trim();
        return this;
    }
