 ...
 */
public abstract class AbstractBaseRestController<T extends Serializable, VO extends BaseVo<T>> {
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/{primaryKeys}", method = RequestMethod.DELETE, produces = "application/json; charset=utf-8")
     ...
     */
    @RequestMapping(value = "/{primaryKey}", method = RequestMethod.GET, produces = "application/json; charset=utf-8")
    public ResponseEntity<T> detail_GET(@PathVariable Long primaryKey) {
        T bean = service.selectByPrimaryKey(primaryKey);
        if (bean == null) {
    ...
    public ResponseEntity<List<T>> list_POST(VO vo) {
